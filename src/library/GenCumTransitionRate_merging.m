function [xTime, TM, TM2] = GenCumTransitionRate_merging(track, tTrial, tCrit)
...
for wInd = 1:length(xInd)
    ...
    if ~isempty(track)
        DiffMat = [0; abs(diff(track(:,1)))]; 
        for iInd = 1:(length(track(:,1))-wInd)
            if sum(DiffMat(iInd:(iInd + wInd))) ~= 0 
                ccnt_s  = ccnt_s  + (track(iInd,1) == 1);  % cw
                ccnt_s2 = ccnt_s2 + (track(iInd,1) == -1); % ccw
            end
